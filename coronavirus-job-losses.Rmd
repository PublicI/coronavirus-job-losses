...
transportation <- list(air_transport, rail_transport, water_transport, transit_ground_transport, scenic_transport, support_activities_transport, all_industries, county_pops) %>% 
  ...
  filter(!is.na(september_employment_lq_transportation)) %>% 
  select(area_fips,
         county,
         population,
...
arts_ent_rec <- list(arts_ent_rec, all_industries, county_pops) %>% 
  reduce(full_join, by = "area_fips") %>% 
  filter(!is.na(september_employment_lq_arts_ent_rec)) %>% 
  select(area_fips,
         county,
         population,
...
accom_food <- list(accom_food, all_industries, county_pops) %>% 
  reduce(full_join, by = "area_fips") %>% 
  filter(!is.na(september_employment_lq_accom_food)) %>% 
  select(area_fips,
         county,
         population,
...
transportation_geo_pal <- colorNumeric("Reds", domain=transportation_geo$september_employment_lq_transportation)
transportation_geo_popup_text <- paste0(transportation_geo$county, " has a location quotient of ", as.character(transportation_geo$september_employment_lq_transportation),".")

arts_ent_rec_geo_pal <- colorNumeric("Reds", domain=arts_ent_rec_geo$september_employment_lq_arts_ent_rec)
arts_ent_rec_geo_popup_text <- paste0(arts_ent_rec_geo$county, " has a location quotient of ", as.character(arts_ent_rec_geo$september_employment_lq_arts_ent_rec),".")

accom_food_geo_pal <- colorNumeric("Reds", domain=accom_food_geo$september_employment_lq_accom_food)
accom_food_geo_popup_text <- paste0(accom_food_geo$county, " has a location quotient of ", as.character(accom_food_geo$september_employment_lq_accom_food),".")

# Map the industries using the September employment location quotient as the color scale
transportation_geo_map <- transportation_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ transportation_geo_pal(transportation_geo$september_employment_lq_transportation),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ transportation_geo_popup_text) %>% 
  addLegend(pal = transportation_geo_pal,
            values = transportation_geo$september_employment_lq_transportation,
            position = "bottomleft",
            title = "Location quotient for the</br/>transportation industry")

arts_ent_rec_geo_map <- arts_ent_rec_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ arts_ent_rec_geo_pal(arts_ent_rec_geo$september_employment_lq_arts_ent_rec),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ arts_ent_rec_geo_popup_text) %>% 
  addLegend(pal = arts_ent_rec_geo_pal,
            values = arts_ent_rec_geo$september_employment_lq_arts_ent_rec,
            position = "bottomleft",
            title = "Location quotient for the arts,</br/>entertainment and recreation industry")

accom_food_geo_map <- accom_food_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ accom_food_geo_pal(accom_food_geo$september_employment_lq_accom_food),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ accom_food_geo_popup_text) %>% 
  addLegend(pal = accom_food_geo_pal,
            values = accom_food_geo$september_employment_lq_accom_food,
            position = "bottomleft",
            title = "Location quotient for the accommodation</br/>and food services industry")

# Export the maps
save_html(transportation_geo_map, file = "transportation_geo_map.html", background = "white")
save_html(arts_ent_rec_geo_map, file = "arts_ent_rec_geo_map.html", background = "white")
save_html(accom_food_geo_map, file = "accom_food_geo_map.html", background = "white")
```