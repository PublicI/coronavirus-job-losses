...
# How many counties are more reliant on leisure and hospitality for jobs than the nation as a whole?
counties_leisure_hospitality_insurance %>% 
  filter(employment_leisure_hospitality_lq > 1) %>% 
  summarize(count = n())

# How many counties have a rate of uninsured residents higher than the nation as a whole?
counties_leisure_hospitality_insurance %>% 
...
# Of these counties, which have a higher non-white population than the nation as a whole?
...
# In which states are these counties?
counties_more_reliant_higher_uninsured_higher_non_white %>% 
  group_by(state_name) %>% 
  summarize(count = n()) %>% 
  arrange(desc(count))
