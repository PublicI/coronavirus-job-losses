...
transportation <- list(air_transport, rail_transport, water_transport, transit_ground_transport, scenic_transport, support_activities_transport, all_industries, county_pops) %>% 
  ...
  mutate(september_employment_transportation = select(., 2, 4, 6, 8, 10, 12) %>% 
         ...
         total_qtrly_wages_transportation = select(., 3, 5, 7, 9, 11, 13) %>% 
           rowSums(na.rm = T),
         september_employment_lc_transportation = round(september_employment_transportation / september_employment_all_industries, digits = 2),
  ...
  filter(!is.na(september_employment_lc_transportation)) %>% 
  select(area_fips,
         county,
         state,
         population,
         ...
         september_employment_lc_transportation,
...
arts_ent_rec <- list(arts_ent_rec, all_industries, county_pops) %>% 
  reduce(full_join, by = "area_fips") %>% 
  mutate(september_employment_lc_arts_ent_rec = round(september_employment_arts_ent_rec / september_employment_all_industries, digits = 2),
         total_qtrly_wages_lc_arts_ent_rec = round(total_qtrly_wages_arts_ent_rec / total_qtrly_wages_all_industries, digits = 2)) %>% 
  filter(!is.na(september_employment_lc_arts_ent_rec)) %>% 
  select(area_fips,
         county,
         state,
         population,
         ...
         september_employment_all_industries,
         total_qtrly_wages_all_industries,
         september_employment_lc_arts_ent_rec,
...
accom_food <- list(accom_food, all_industries, county_pops) %>% 
  reduce(full_join, by = "area_fips") %>% 
  mutate(september_employment_lc_accom_food = round(september_employment_accom_food / september_employment_all_industries, digits = 2),
         total_qtrly_wages_lc_accom_food = round(total_qtrly_wages_accom_food / total_qtrly_wages_all_industries, digits = 2)) %>% 
  filter(!is.na(september_employment_lc_accom_food)) %>% 
  select(area_fips,
         county,
         state,
         population,
         ...
         september_employment_all_industries,
         total_qtrly_wages_all_industries,
         september_employment_lc_accom_food,
...
selected_industries <- list(transportation, arts_ent_rec, accom_food) %>% 
  ...
  mutate(september_employment_selected_industries = select(., 5, 16, 27) %>% 
         ...
         total_qtrly_wages_selected_industries = select(., 6, 17, 28) %>% 
           rowSums(na.rm = T),
         september_employment_lc_selected_industries = round(september_employment_selected_industries / september_employment_all_industries, digits = 2),
  ...
  filter(!is.na(september_employment_lc_selected_industries)) %>% 
  select(area_fips,
         county,
         state,
         population,
         ...
         september_employment_all_industries,
         total_qtrly_wages_all_industries,
         september_employment_lc_selected_industries,
...
transportation_geo_pal <- colorNumeric("Reds", domain=transportation_geo$september_employment_lc_transportation)
transportation_geo_popup_text <- paste0(transportation_geo$county, " has a local concentration of ", as.character(transportation_geo$september_employment_lc_transportation),".")

arts_ent_rec_geo_pal <- colorNumeric("Reds", domain=arts_ent_rec_geo$september_employment_lc_arts_ent_rec)
arts_ent_rec_geo_popup_text <- paste0(arts_ent_rec_geo$county, " has a local concentration of ", as.character(arts_ent_rec_geo$september_employment_lc_arts_ent_rec),".")

accom_food_geo_pal <- colorNumeric("Reds", domain=accom_food_geo$september_employment_lc_accom_food)
accom_food_geo_popup_text <- paste0(accom_food_geo$county, " has a local concentration of ", as.character(accom_food_geo$september_employment_lc_accom_food),".")

selected_industries_geo_pal <- colorNumeric("Reds", domain=selected_industries_geo$september_employment_lc_selected_industries)
selected_industries_geo_popup_text <- paste0(selected_industries_geo$county, " has a local concentration of ", as.character(selected_industries_geo$september_employment_lc_selected_industries),".")

# Map the industries using the September employment local concentration as the color scale
transportation_geo_map <- transportation_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ transportation_geo_pal(transportation_geo$september_employment_lc_transportation),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ transportation_geo_popup_text) %>% 
  addLegend(pal = transportation_geo_pal,
            values = transportation_geo$september_employment_lc_transportation,
            position = "bottomleft",
            title = "Local concentration for the</br/>transportation industry")

arts_ent_rec_geo_map <- arts_ent_rec_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ arts_ent_rec_geo_pal(arts_ent_rec_geo$september_employment_lc_arts_ent_rec),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ arts_ent_rec_geo_popup_text) %>% 
  addLegend(pal = arts_ent_rec_geo_pal,
            values = arts_ent_rec_geo$september_employment_lc_arts_ent_rec,
            position = "bottomleft",
            title = "Local concentration for the arts,</br/>entertainment and recreation industry")

accom_food_geo_map <- accom_food_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ accom_food_geo_pal(accom_food_geo$september_employment_lc_accom_food),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ accom_food_geo_popup_text) %>% 
  addLegend(pal = accom_food_geo_pal,
            values = accom_food_geo$september_employment_lc_accom_food,
            position = "bottomleft",
            title = "Local concentration for the accommodation</br/>and food services industry")

selected_industries_geo_map <- selected_industries_geo %>% 
  leaflet() %>% 
  addTiles() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(fillColor = ~ selected_industries_geo_pal(selected_industries_geo$september_employment_lc_selected_industries),
              fillOpacity = 0.5,
              smoothFactor = 0.5,
              stroke = F,
              popup = ~ selected_industries_geo_popup_text) %>% 
  addLegend(pal = selected_industries_geo_pal,
            values = selected_industries_geo$september_employment_lc_selected_industries,
            position = "bottomleft",
            title = "Local concentration for</br/>selected industries")
...
transportation %>% 
  filter(september_employment_lc_transportation >= .10)
