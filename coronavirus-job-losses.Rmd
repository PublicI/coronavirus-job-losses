...
# Import the 4/9/20 weekly uninsurance claims report from (the DoL)[https://oui.doleta.gov/press/2020/040920.pdf]
ui_claims <- read_excel("data/dol_weekly_ui_claims_report_2020_04_09.xlsx", sheet = 2) %>% 
  filter(state_supplied_comment != "No comment." & state_supplied_comment != "No comments.")
...
# How many counties with a population of at least 50,000 are at least as reliant on leisure and hospitality for jobs as the nation as a whole?
counties_leisure_hospitality_insurance %>% 
  filter(pop_total >= 50000 & employment_leisure_hospitality_lq >= 1) %>% 
  summarize(count = n())

# How many counties with a population of at least 50,000 have a rate of uninsured residents at least as high as the nation as a whole?
counties_leisure_hospitality_insurance %>% 
  filter(pop_total >= 50000 & pct_uninsured_total >= us_pct_uninsured_total ) %>% 
  summarize(count = n())

# Which counties meet both conditions?
counties_more_reliant_higher_uninsured <- counties_leisure_hospitality_insurance %>% 
  filter(pop_total >= 50000 & employment_leisure_hospitality_lq >= 1 & pct_uninsured_total >= us_pct_uninsured_total)
...
counties_more_reliant_higher_uninsured_higher_non_white <- counties_more_reliant_higher_uninsured %>% 
  filter(pop_total >= 50000 & pct_non_white >= us_pct_non_white)
...
# Of the 35 states that offered comments to the Department of Labor in the 4/9/20 UI claims report, how many mentioned leisure and hospitality?
leisure_hospitality_mentioned_ui_claims <- ui_claims %>% 
  filter(str_detect(state_supplied_comment, "accommodation and food services") |
           str_detect(state_supplied_comment, "arts, entertainment, and recreation"))
